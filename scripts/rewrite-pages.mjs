...
const files = {
  'src/pages/ProjectAnalysis.tsx': String.raw`import { useEffect, useState } from 'react'
...
export function ProjectAnalysis() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const voteSummary = items.map((item) => ({
        item_id: item.id,
      ...
      const saved = localApi.saveAnalysis({
      ...
      <div className="p-6">
        {!analysis ? (
          ...
          <div className="mx-auto max-w-4xl space-y-8">
            ...
            <section>
              <h2 className="mb-4 text-lg font-semibold">시안별 의견 요약</h2>
            ...
            <section className="card p-5">
              <h2 className="mb-4 text-lg font-semibold">감성 분석</h2>
            ...
            <section className="card p-5">
              <h2 className="mb-4 text-lg font-semibold">브랜드 적합도</h2>
              <div className="space-y-3">
                {items.map((item) => {
                  const score = analysis.brand_fit_scores[item.id] ?? 0
                  return (
                    <div key={item.id} className="flex items-center gap-3">
                      <span className="w-28 truncate text-sm">{item.title}</span>
