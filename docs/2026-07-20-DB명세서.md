...
- 작성일: 2026-07-20
- 기준: `supabase/migrations/001`–`018`, `src/types/index.ts`
...
| notification_prefs | JSONB | N | 아래 참고 | 알림 수신 설정 |
| created_at | TIMESTAMPTZ | N | `now()` | |
| updated_at | TIMESTAMPTZ | N | `now()` | |

**인덱스:** `idx_users_workspace (workspace_id)`

**notification_prefs 기본값**
...
| deadline | TIMESTAMPTZ | N | | 투표/진행 마감 |
...
파일 메타는 `item_versions.file_url` / `thumbnail_url`에 URL로 저장.
...
이 문서는 2026-07-20 시점 마이그레이션 스냅샷입니다. 스키마 변경 시 본 문서와 SQL을 함께 갱신하세요.
