  ...
  return (
    <div className="flex min-h-[calc(100vh-3.5rem)] flex-col">
      ...
      <div className="mx-auto w-full max-w-2xl p-6">
        ...
        <h2 className="mb-3 mt-8 text-lg font-bold">?? ??</h2>
        <ul className="space-y-2 text-sm text-ink-muted">
          {lines.flatMap((line) =>
            (line.actions ?? []).map((a) => (
              <li key={a.id}>
                {new Date(a.created_at).toLocaleDateString('ko-KR')}{' '}
                {line.approvers?.find((u) => u.id === a.user_id)?.name} ? {line.step_order}??{' '}
                {a.action === 'approved' ? '??' : '??'}
              </li>
            ))
          )}
          {lines.every((l) => !l.actions?.length) && <li>??? ????</li>}
        </ul>
      </div>
    </div>
  )
      ...
      <div className="mx-auto w-full max-w-3xl p-6">
              ...
              <div key={item.id} className="card overflow-hidden">
                ...
                <div className="p-3 text-sm">
                  ...
                  <p className="text-ink-muted">
                    {item.vote_count ?? 0}? - ?{item.avg_score ?? 0}
                  </p>
                </div>
              </div>
            ))}
        ...
        <section className="card mt-6 p-4">
          ...
          {sortedItems.slice(0, 3).map((item, i) => (
            <p key={item.id} className="text-sm text-ink-muted">
              {i + 1}?: {item.title} ({item.vote_rate ?? 0}%)
            </p>
          ))}
        </section>
        ...
        {analysis && (
          <section className="card mt-4 border-accent/20 bg-accent-soft/40 p-4">
            <h3 className="mb-2 font-semibold">AI ?? ??</h3>
        ...
        >
          <Textarea
            ...
            placeholder="??? ??? ??? ????? ??????. (?? 10?)"
          />
          <p className="mt-1 text-xs text-ink-muted">?? ?: {rejectReason.length}/500</p>
        </Modal>
        ...
        <ConfirmDialog
          ...
          onConfirm={approve}
          title="?????????"
          description={`?? ?? ??: ${items.find((i) => i.id === selectedItem)?.title ?? '??'}. ? ??? ??? ? ????.`}
          confirmLabel="?? ??"
          loading={loading}
        />
      </div>
